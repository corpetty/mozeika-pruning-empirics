"""
Experiment 10: Spectral structure under pruning.
Architecture: 20 -> 16 -> 1 (MLP with tanh in hidden)
5 seeds, rho ∈ {0, 0.0003, 0.0005, 0.001}
"""
import numpy as np
import os
...
def mlp_sample(M, layer_sizes, sigma=0.01, seed=None):
    ...
    # Targets: tanh in hidden, linear output
    a = X.copy()
    ...
    y = a.flatten().reshape(-1, 1)  # Ensure (M, 1) shape to avoid broadcast issues
    return X, y, w0_list, h0_list


def grad_mlp_loss_w(w_list, h_list, X, y):
    """Backprop: simple and correct."""
    M = X.shape[0]
    n_layers = len(w_list)
    ...
    grads = []
    delta = a_list[-1] - y  # (M, 1) - both are now (M, 1)
    
    for l in range(n_layers - 1, -1, -1):
        a_prev = a_list[l]  # (M, d_in)
        
        # Grad for W_l: a_prev.T @ delta * h
        delta_2d = delta  # (M, 1)
        grad = a_prev.T @ delta_2d  # (d_in, M) @ (M, 1) = (d_in, 1)
        grad = grad * h_list[l]  # element-wise
        
        grads.insert(0, grad)
        
        # Backprop delta to previous layer
        if l > 0:
            # delta_prev = delta_l @ W_l.T * tanh'(z_{l-1})
            delta_prev = delta_2d @ w_list[l].T  # (M, 1) @ (1, d_in) = (M, d_in)
            delta_prev = delta_prev * (1 - z_list[l - 1] ** 2)  # tanh'
            delta = delta_prev  # keep as (M, d_in) for next iteration
...
def main():
    ...
    for rho in rhos:
        ...
        for seed in range(n_seeds):
            ...
            print(f"  seed {seed}: loss = {np.mean(losses):.4f}")
