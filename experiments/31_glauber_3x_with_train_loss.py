"""
31_glauber_3x_with_train_loss.py

Re-run of the 3x Glauber experiment (LeNet-300-100, MNIST) with training loss
logged at every round alongside the existing test metrics.

Config: 3 sweeps/temp-step, linear T anneal 1e-7 → 0 over 20 steps,
        max_rounds=60, target_sparsity=0.99

Output: results/glauber_3x_with_train_loss.json
"""
...
class Cfg:
    ...
    seed: int = 0
    # Temperature schedule: linear anneal from T_start to 0 over temp_steps
    T_start: float = 1e-7
    temp_steps: int = 20        # temperature changes every (max_rounds // temp_steps) rounds
    ...
    out_path: str = os.path.join(os.path.dirname(__file__), "..", "results",
                                  "glauber_3x_with_train_loss.json")

    def T_at(self, round_idx: int) -> float:
        """Linear anneal: T_start at round 1, 0 at round temp_steps*(rounds/temp_steps)."""
...
def train_epoch(model, loader, cfg) -> float:
    """Train one epoch; returns mean CE loss (no L2) over all batches."""
    model.train()
...
def train_model(model, loader, cfg, epochs) -> List[float]:
    """Train `epochs` epochs; return list of per-epoch mean CE training loss."""
    return [train_epoch(model, loader, cfg) for _ in range(epochs)]
...
def main():
    ...
    # Round 0 — pretrain
    train_losses = train_model(model, train_loader, cfg, cfg.train_epochs_per_round)
    E, ce, l2, rho = energy(model, test_loader, cfg)
    _, acc = evaluate(model, test_loader, cfg.device)
    spar = global_sparsity(model)
    rec = {
        "round": 0, "T": float("nan"),
        "train_loss": train_losses[-1],          # last pretrain epoch
        "train_loss_history": train_losses,
        "test_ce": ce, "test_acc": acc,
        "energy": E, "ce": ce, "l2": l2, "rho": rho,
        "sparsity": spar, "dead": 0,
        "pruned": 0, "regrown": 0,
    }
    records.append(rec)
    print(f"R0 | spar={spar:.4f} | acc={acc:.4f} | train_loss={train_losses[-1]:.4f} | test_ce={ce:.4f}")

    for r in range(1, cfg.max_rounds + 1):
        T = cfg.T_at(r)

        # Pre-sweep training
        pre_losses = train_model(model, train_loader, cfg, cfg.train_epochs_per_round)
        ...
        post_losses = train_model(model, train_loader, cfg, cfg.finetune_epochs)
        ...
        # Training loss = last post-sweep finetune epoch (closest to what the model "is" now)
        train_loss_final = post_losses[-1]
        all_train_losses = pre_losses + post_losses

        rec = {
            "round": r, "T": T,
            "train_loss": train_loss_final,
            "train_loss_pre": pre_losses[-1],
            "train_loss_post": post_losses[-1],
        ...
        records.append(rec)
        gap = train_loss_final - ce   # positive = overfitting gap (unlikely here)
        print(f"R{r:02d} | T={T:.2e} | spar={spar:.4f} | acc={acc:.4f} | "
              f"train_loss={train_loss_final:.4f} | test_ce={ce:.4f} | gap={gap:+.4f} | "
