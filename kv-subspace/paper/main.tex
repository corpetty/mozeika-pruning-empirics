...
calibration should suffice for compression. Our Experiment 1 confirms this---K
...
We combine subspace projection with \textbf{PolarQuant}
...
Kaushik \etal~\cite{kaushik2025uwsh} demonstrate that deep networks trained on
over 1,100 diverse tasks---from ViTs to LLaMA-3 LoRAs---converge to a shared
low-dimensional parameter subspace. The hypothesis predicts that per-component
PCA bases are stable and transferable. We extend this inquiry to \emph{KV
activation} vectors, verifying whether the same subspace stability holds at
inference time (Experiment~1). Our principal-angle analysis confirms it for K
vectors: cross-domain overlap $0.70$, cross-layer overlap $0.56$. V vectors show
higher cross-layer overlap ($0.74$) but resist compression---a tension between
basis stability and compression viability that we resolve in Experiments~19--21.
...
\subsection{Subspace Geometry: Testing the UWSH (Exp.~1)}
\label{sec:uwsh}

Motivated by \citet{kaushik2025uwsh}, we first verify that KV activation vectors
exhibit the stable low-dimensional subspace structure required for offline
compression. We compute principal angles between PCA subspaces (90\% variance
threshold) for three comparison types on Qwen3-14B-AWQ: \emph{cross-layer}
(adjacent layers), \emph{cross-head} (head pairs within a layer), and
\emph{cross-domain} (PCA fitted on fiction vs.\ factual text).

\textbf{Results.} K cross-domain overlap $= 0.703$; K cross-layer overlap $=
0.565$ (rising to $0.650$ in layers 10--39); K cross-head overlap $= 0.456$.
V cross-layer overlap $= 0.736$; V cross-domain overlap $= 0.640$.

\textbf{Implications.} Cross-domain overlap of $0.70$ for K vectors confirms that
a single offline calibration corpus generalizes across deployment domains ---
validating our approach. Cross-head overlap of $0.46$ confirms that
\emph{per-head bases are necessary}; sharing across heads would lose significant
structure. The higher V cross-layer overlap ($0.74$ vs $0.57$ for K) is
consistent with V having a more globally uniform geometry, yet one that resists
truncation (see Section~\ref{sec:v_failure}).

\paragraph{Protocol.} All subsequent experiments use three evaluation passages (biology,
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.52\textwidth]{fig1_bitrate_heatmap.pdf}
  \caption{Rel.\ PPL heatmap for the $k \times$ bit-depth sweep on Qwen3-14B-AWQ.
  Green borders mark viable configs ($\leq$1.20$\times$). Columns within the same
  row (fixed $k$) vary by at most $0.68\times$, while rows within the same column
  (fixed bits) vary by up to $2.06\times$, confirming that truncation error
  dominates quantization noise.}
  \label{fig:heatmap}
\end{figure}

Figure~\ref{fig:heatmap} summarizes the full sweep.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.58\textwidth]{fig3_cross_arch.pdf}
  \caption{K compression rel.\ PPL at $k{=}112$/4-bit across architectures.
  Compression ratio (CR) annotated above each bar. All four architectures pass
  the 20\% budget (dashed line). Mistral and Phi-4 achieve higher CR because
  they tolerate more truncation.}
  \label{fig:cross_arch}
\end{figure}

Figure~\ref{fig:cross_arch} compares all four architectures at $k{=}112$/4-bit.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.62\textwidth]{fig2_long_context.pdf}
  \caption{Relative PPL vs.\ context length for four compression configs on a
  40K-token slice of War and Peace. $k{=}128$/4-bit (blue) is flat and well below
  the 20\% budget (dashed). $k{=}64$/4-bit (red) improves with context as the
  calibrated basis becomes more representative, but never recovers to
  viability.}
  \label{fig:long_ctx}
\end{figure}

As shown in Figure~\ref{fig:long_ctx}, $k{=}128$/4-bit is stable within
$1.05$--$1.11{\times}$ across all context lengths with no accumulation.
Per-position analysis confirms compression errors are uniformly distributed
across token positions.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.52\textwidth]{fig8_basis_drift.pdf}
  \caption{Mean PCA basis overlap (cosine$^2$ of principal angles) between the
  early context window (0--2K tokens) and progressively later windows. K
  (blue) drifts slowly and stabilizes. V (red) drifts more but also
  stabilizes, justifying offline calibration.}
  \label{fig:drift}
\end{figure}

\textbf{Basis drift.} Figure~\ref{fig:drift} traces PCA basis overlap between
early (tokens 0--2K) and progressively later windows.
Final overlap at 35K--40K: K $= 0.825$, V $= 0.702$.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.80\textwidth]{fig6_needle.pdf}
  \caption{Needle-in-haystack retrieval accuracy (fact inserted at 5 depths ×
  4 context lengths). Green = correct, red = incorrect. $k{=}128$/4-bit (center)
  equals or exceeds baseline at every cell. $k{=}96$/4-bit (right) collapses at
  32K context.}
  \label{fig:needle}
\end{figure}

Figure~\ref{fig:needle} shows per-cell retrieval accuracy across context lengths
and needle depths. $k{=}128$/4-bit matches or exceeds baseline accuracy at all
context lengths, achieving $97\%$ overall vs.\ $93\%$ baseline. The marginal improvement at 16K is
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{fig5_v_failure.pdf}
  \caption{\emph{Left:} V compression scan on Qwen3-14B-AWQ with K fixed at
  $k{=}112$/4-bit. Rel.\ PPL (log scale) remains far above the 20\% budget for
  all $k_V < 128$. \emph{Right:} K vs.\ V compression on Llama-3.1-8B (no
  QK-norm). V-only at $k{=}112$ gives $12.14{\times}$ PPL; even $k_V{=}124$
  ($1.6\%$ truncation) gives $3.45{\times}$. The failure is architecture-independent.}
  \label{fig:v_fail}
\end{figure}

Figure~\ref{fig:v_fail} presents the V threshold scan and cross-architecture
comparison. The QK-norm hypothesis is \textbf{rejected}: Llama-3.1 without
QK-norm shows identical V compression failure---$k_V{=}112$ produces $12.14{\times}$ PPL,
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.70\textwidth]{fig4_layer_sensitivity.pdf}
  \caption{Per-layer $\Delta$PPL under K ablation ($k{=}64$/4-bit, one layer at a time)
  on Qwen3-14B-AWQ. Red bars: compression hurts. Green bars: compression
  helps (mild regularization). Layer 37 is the most sensitive by a large
  margin.}
  \label{fig:layer_sens}
\end{figure}

\paragraph{Layer sensitivity (Exp.~16).}
Figure~\ref{fig:layer_sens} shows per-layer $\Delta$PPL from ablating each of
40 layers at $k{=}64$/4-bit, revealing strong heterogeneity. Layer
...
\begin{figure}[h]
  ...
  \caption{Cross-domain calibration transfer: rel.\ PPL for calib$\to$eval domain
  pairs. \emph{Left:} $k{=}128$/4-bit is robust (all cells near 1.0).
  \emph{Right:} $k{=}96$/4-bit is brittle---some off-diagonal pairs exceed 2.5$\times$.
  Universal calibration (mixed corpus) is the safest strategy for aggressive $k$.}
  \label{fig:cross_domain}
\end{figure}

Figure~\ref{fig:cross_domain} visualizes the calib$\to$eval transfer matrices
for both configs. \textbf{Practical recommendation:} A single calibration pass
on a diverse 2K-token corpus generalizes safely at $k{=}128$. For aggressive $k{=}96$, universal
