...
# Import helper to extract KV from any cache format
from scripts.generate_gemma4_basis import extract_kv_by_layer
...
class SubRotQCache:
    ...
    def __init__(self, basis_path, k=128, n_bits=4, device='cuda'):
        ...
        self.bases = {}
        self.means = {}
        self.head_dims = {}  # (layer, head) → actual_d_head for this head
        self.head_ks = {}    # (layer, head) → actual k for this head
        ...
        for key in data.files:
            if key.startswith('U_L') and not key.startswith('U_V'):
                ...
                layer = int(parts[1][1:])
                head = int(parts[2][1:])
                basis = torch.from_numpy(data[key]).to(device, torch.float32)
                self.bases[(layer, head)] = basis
                
                # Load per-head metadata if available (new format)
                d_head_key = f'd_head_L{layer}_H{head}'
                k_key = f'k_L{layer}_H{head}'
                if d_head_key in data.files:
                    self.head_dims[(layer, head)] = int(data[d_head_key][0])
                else:
                    # Fallback: infer from basis shape (old format)
                    self.head_dims[(layer, head)] = basis.shape[0]
                
                if k_key in data.files:
                    self.head_ks[(layer, head)] = int(data[k_key][0])
                else:
                    # Fallback: infer from basis shape
                    self.head_ks[(layer, head)] = basis.shape[1]
        ...
        if self.head_dims:
            dim_counts = {}
            for d in self.head_dims.values():
                dim_counts[d] = dim_counts.get(d, 0) + 1
            dims_str = ", ".join([f"d={d}:{cnt}heads" for d, cnt in sorted(dim_counts.items())])
            print(f"  Loaded {len(self.bases)} K bases ({dims_str})")
        else:
    ...
    def compress_and_store(self, past_key_values):
        ...
        # Extract KV from any cache format (DynamicCache/HybridCache/tuple)
        kv_layers = extract_kv_by_layer(past_key_values)
    ...
    def decompress(self):
        """Decompress K, return DynamicCache-compatible past_key_values."""
        cache = DynamicCache()
        
        for layer_idx, (K_c, V) in enumerate(self.cache):
            batch, num_heads, seq_len, _ = K_c.shape
            
            # Infer actual d_head from V (not compressed, preserves true head dimension)
            actual_d_head = V.shape[-1]
            
            K = torch.zeros(batch, num_heads, seq_len, actual_d_head, 
            ...
            for h in range(num_heads):
                ...
                U = self.bases[key]  # (expected_d_head, k)
                expected_d_head = self.head_dims.get(key, U.shape[0])
                mean = self.means.get(key, torch.zeros(expected_d_head, device=K_c.device))
                
                # Reconstruct: K = K_proj @ U^T + mean
                K_proj_h = K_c[:, h, :, :].to(torch.float32)  # (batch, seq_len, k)
                K_recon = K_proj_h @ U.T  # (batch, seq_len, expected_d_head)
                K_recon = K_recon + mean.view(1, 1, -1)  # Broadcast mean
                
                # Handle heterogeneous head dims (should match if basis generated correctly)
                if expected_d_head != actual_d_head:
                    # This shouldn't happen with correct basis, but handle gracefully
                    if expected_d_head < actual_d_head:
                        pad_size = actual_d_head - expected_d_head
                        K_recon = torch.cat([K_recon, torch.zeros_like(K_recon[..., :pad_size])], dim=-1)
                    else:
                        K_recon = K_recon[..., :actual_d_head]
                
                K[:, h, :, :] = K_recon.to(torch.bfloat16)
            
            cache.update(K, V.to(torch.bfloat16), layer_idx)
...
def calculate_perplexity(model, tokenizer, text, use_subrotq=False, subrotq_cache=None, max_length=256):
    """Calculate perplexity on a text sample using autoregressive generation.
    
    This properly tests cache quality by generating tokens one-by-one.
    """
    ...
    if seq_len < 10:
        return float('inf')  # Skip very short sequences
    ...
    with torch.no_grad():
        for i in range(1, seq_len):
            ...
            if use_subrotq and past_key_values is not None:
                # Decompress cache for this step
                past_kv_decompressed = subrotq_cache.decompress()
                
                # Forward with decompressed cache
                outputs = model(
