#!/usr/bin/env python3
"""
VGG16 CIFAR-10 — Pi5 Inference
99.19% sparse, fine-tuned, INT8 quantized
Accuracy: ~89.0%  |  Size: 37 MB  |  Speed: ~190 ms/image (Pi5 CPU)

Requirements:
  pip install onnxruntime pillow numpy

Usage:
  python3 infer.py image.jpg
  python3 infer.py image.jpg --model vgg16_finetuned_99pct_compact_int8_static.onnx
  python3 infer.py --benchmark          # time 100 runs on a random input
"""
...
def main():
    parser = argparse.ArgumentParser(description="VGG16 CIFAR-10 Pi5 inference")
    parser.add_argument("image",        nargs="?", help="Path to input image")
    parser.add_argument("--model",      default=os.path.join(
                            os.path.dirname(__file__),
                            "vgg16_finetuned_99pct_compact_int8_static.onnx"),
    ...
    if not os.path.exists(args.model):
        ...
        print("Place vgg16_finetuned_99pct_compact_int8_static.onnx next to this script.")
        sys.exit(1)
