...
class VGGPruningConfig:
    ...
    max_pruning_rounds: int = 60
    fisher_batches: int = 3
    eta_w: float = 1e-5
    eta_b: float = 1e-5
    # Rho values scaled up ~100x vs v2 to drive pruning into 99% sparsity regime.
    # At 99% sparsity the surviving weights must have high Fisher saliency;
    # we need rho large enough to declare the long tail of low-saliency weights
    # prunable, while still leaving a 1% residual of high-saliency survivors.
    rho_conv_w: float = 1e-6
    rho_conv_b: float = 1e-6
    rho_fc_w: Tuple[float, float, float] = (5e-6, 5e-6, 1e-6)
    rho_fc_b: Tuple[float, float] = (5e-6, 5e-6)  # output bias unpruned
    # Relaxed cap: allow up to 30% of active weights to be pruned per round
    # (v2 used 15%; at 99% target the long tail needs to drain faster).
    prune_fraction_cap: Optional[float] = 0.30
...
@torch.no_grad()
def compute_sparsity_penalty(model: nn.Module, cfg: VGGPruningConfig) -> float:
    """Exact (1/2) * sum_i rho_i * h_i, summed over all prunable parameters.

    For each parameter i that is currently active (h_i=1), we add rho_i/2 * 1.
    Since h_i is binary (0 or 1 via the mask), this reduces to:
        (1/2) * rho_w * active_weight_count_for_layer  (per layer)
    """
    penalty = 0.0
    conv_index = 0
    fc_index = 0
    for name, m in named_masked_modules(model):
        if isinstance(m, MaskedConv2d):
            ...
            rho_b = cfg.rho_fc_b[fc_index] if fc_index < 2 else 0.0

        # weight mask contribution
        active_w = int(m.weight_mask.sum().item())
        penalty += 0.5 * rho_w * active_w

        # bias mask contribution (if prunable)
        if m.bias_mask is not None:
            active_b = int(m.bias_mask.sum().item())
            penalty += 0.5 * rho_b * active_b
...
def compute_full_energy(
    ...
    """Compute E = CE + L2 + sparsity_penalty (all normalised per sample for CE/L2).

    Returns:
        (ce_loss, l2_penalty, sparsity_penalty, total_energy)
    """
    model.eval()
    total_ce = 0.0
    total_l2 = 0.0
    ...
    for batch_idx, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            l2 = masked_l2_penalty(model, cfg).item() * y.size(0)  # scale per sample
        total_ce += ce
        total_l2 += l2
        total_samples += y.size(0)

    ce_norm = total_ce / max(1, total_samples)
    l2_norm = total_l2 / max(1, total_samples)
    sp = compute_sparsity_penalty(model, cfg)
    energy = ce_norm + l2_norm + sp
    return ce_norm, l2_norm, sp, energy
...
def prune_round(model: nn.Module, fisher: Dict[str, torch.Tensor], cfg: VGGPruningConfig) -> Dict[str, int]:
    ...
    Additions vs v1:
    - ``energy_descent_directions``: total number of active parameters whose
      saliency < rho/2 *before* the fraction cap is applied.  This is the
      exact count of coordinates along which the energy would decrease if
      pruned.  It is the size of the full uncapped candidate set.
    - ``energy_descent_pruned``: how many of those were actually pruned
      (after the cap).  The difference is the residual descent budget left
      at the end of the round.
    """
    stats: Dict[str, int] = {}
    total_descent_dirs = 0   # uncapped: sal < rho/2
    total_actually_pruned = 0
    ...
    for name, m in named_masked_modules(model):
        if isinstance(m, MaskedConv2d):
            ...
            # Count ALL energy-decreasing weight directions (uncapped)
            all_cand_w = active_w & (sal_w < (rho_w / 2.0))
            total_descent_dirs += int(all_cand_w.sum().item())

            # Apply fraction cap
            cand_w = all_cand_w.clone()
            if cfg.prune_fraction_cap is not None and cand_w.any():
                thr = _fraction_threshold(sal_w[active_w], cfg.prune_fraction_cap)
                if thr is not None:
                    cand_w &= (sal_w <= thr)
            pruned_w = m.prune_weights_(cand_w)
            stats[f"conv{conv_index}.weights_pruned"] = pruned_w
            total_actually_pruned += pruned_w
            ...
            if m.bias_mask is not None:
                ...
                all_cand_b = active_b & (sal_b < (rho_b / 2.0))
                total_descent_dirs += int(all_cand_b.sum().item())

                cand_b = all_cand_b.clone()
                if cfg.prune_fraction_cap is not None and cand_b.any():
                    thr = _fraction_threshold(sal_b[active_b], cfg.prune_fraction_cap)
                    if thr is not None:
                        cand_b &= (sal_b <= thr)
                pruned_b = m.prune_biases_(cand_b)
                stats[f"conv{conv_index}.biases_pruned"] = pruned_b
                total_actually_pruned += pruned_b
            ...
            if cfg.prune_fraction_cap is not None and cand_w.any():
                ...
                if thr is not None:
                    cand_w &= (sal_w <= thr)
            pruned_w = m.prune_weights_(cand_w)
            stats[f"fc{fc_index + 1}.weights_pruned"] = pruned_w
            total_actually_pruned += pruned_w
            ...
            if m.bias_mask is not None and rho_b is not None:
                ...
                if cfg.prune_fraction_cap is not None and cand_b.any():
                    ...
                    if thr is not None:
                        cand_b &= (sal_b <= thr)
                pruned_b = m.prune_biases_(cand_b)
                stats[f"fc{fc_index + 1}.biases_pruned"] = pruned_b
                total_actually_pruned += pruned_b

            fc_index += 1

    stats["energy_descent_directions"] = total_descent_dirs
    stats["energy_descent_pruned"] = total_actually_pruned
    stats["energy_descent_residual"] = total_descent_dirs - total_actually_pruned
...
# Main experiment
# -----------------------------
def run_pruning_experiment(cfg: VGGPruningConfig, resume_checkpoint: Optional[str] = None):
    set_seed(cfg.seed)
    train_loader, test_loader = build_cifar10_loaders(cfg)

    if resume_checkpoint is not None:
        print(f"Resuming from checkpoint: {resume_checkpoint}")
        ckpt = torch.load(resume_checkpoint, map_location=cfg.device)
        # Build model WITHOUT pretrained download — checkpoint has full state
        cfg_nopretrained = VGGPruningConfig(**{**cfg.__dict__, "use_pretrained": False})
        model = make_masked_vgg16(cfg_nopretrained).to(cfg.device)
        # Load masked state dict — includes weight values + masks from prior run
        missing, unexpected = model.load_state_dict(ckpt["masked_state_dict"], strict=False)
        if missing:
        ...
        ce0, l2_0, sp0, e0 = compute_full_energy(model, test_loader, cfg)
        print(
            f"Resumed | test loss={test_loss:.4f} | test acc={test_acc:.4f} | "
            f"sparsity={report['global_prunable_sparsity']:.4f} | "
    ...
    if resume_checkpoint is None:
        # Energy at round 0 (no pruning yet, use test loader sample for CE/L2)
        ce0, l2_0, sp0, e0 = compute_full_energy(model, test_loader, cfg)
        print(
            f"Round 0 | test loss={test_loss:.4f} | test acc={test_acc:.4f} | "
            f"sparsity={report['global_prunable_sparsity']:.4f} | "
            f"E={e0:.6f} (CE={ce0:.4f} L2={l2_0:.6f} SP={sp0:.4f})"
        )
    ...
    for round_idx in range(1, cfg.max_pruning_rounds + 1):
        ...
        ce, l2, sp, energy = compute_full_energy(model, test_loader, cfg)
        n_dirs   = stats.get("energy_descent_directions", "N/A")
        n_pruned = stats.get("energy_descent_pruned", "N/A")
        n_resid  = stats.get("energy_descent_residual", "N/A")

        print(
            f"Round {round_idx} | test loss={test_loss:.4f} | test acc={test_acc:.4f} | "
            f"sparsity={report['global_prunable_sparsity']:.4f} | "
            f"E={energy:.6f} (CE={ce:.4f} L2={l2:.6f} SP={sp:.4f}) | "
            f"descent_dirs={n_dirs} pruned={n_pruned} residual={n_resid}"
        )
        print(f"  stats: {stats}")
        ...
        if all(v == 0 for k, v in stats.items() if "pruned" in k):
            print("  >> No energy-decreasing directions remain. Converged.")
            break
    ...
    out_path = "vgg16_pruned_and_compressed_v3_95pct.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-root", type=str, default=None, help="Path to CIFAR-10 data root directory")
    args = parser.parse_args()

    cfg = VGGPruningConfig(device=device)
    if args.data_root is not None:
        cfg.data_root = args.data_root
    run_pruning_experiment(cfg, resume_checkpoint=args.resume)
