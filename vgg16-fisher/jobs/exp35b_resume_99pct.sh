#!/bin/bash
#SBATCH --job-name=ai-research/exp35b-resume-99pct
...
# Resume from r30 checkpoint (87% sparsity) — push to 99%
python3 experiments/35_vgg16_perlayer_rho.py \
    ...
    --fisher-batches 5 \
    --seed 0 \
    --resume /home/petty/pruning-research/vgg16-fisher/vgg16_exp35_r30.pt

echo "Pruning job finished at $(date)"

# ── Post-pruning: compact + ONNX + INT8 export (mirrors OBD vgg16 final artifacts) ──
...
sys.path_dir = "/home/petty/pruning-research"
...
cfg = VGGPruningConfig(device=device, data_root="/home/petty/.openclaw/workspace-ai-research/data",
                       use_pretrained=False)
_, test_loader = build_cifar10_loaders(cfg)

# Load final masked model
...
# Save masked checkpoint with metadata (mirrors vgg16_finetuned_99pct.pt naming)
...
summary = {
    ...
    "gap_to_dense_pp": round((0.8994 - acc_int8)*100, 3),
    "comparison_obd": {
