...
def glauber_update_parameters(
    ...
    """
    Finite-temperature Glauber updates for parameter masks h.

    Normalized energy difference:
      Δ̃E = (2h - 1) * ( S / (rho/2 + eps) - 1 )

    Flip probability:
      P_flip = 1 / (1 + exp(beta_h * Δ̃E))

    A positive Δ̃E means the current state is favored.
    """
    named_masks = model.named_mask_tensors()

    active_sites: List[Tuple[str, Tuple[int, ...]]] = []
    inactive_sites: List[Tuple[str, Tuple[int, ...]]] = []

    for name, mask in named_masks.items():
        idx_all = torch.nonzero(torch.ones_like(mask), as_tuple=False)
        for idx in idx_all:
            idx_tuple = tuple(int(v.item()) for v in idx)
            if mask[idx_tuple] > 0.5:
                active_sites.append((name, idx_tuple))
            else:
                inactive_sites.append((name, idx_tuple))

    total_sites = len(active_sites) + len(inactive_sites)
    n_visit = int(math.floor(cfg.h_site_fraction * total_sites))
    if n_visit <= 0 or total_sites == 0:
        return {name: 0 for name in named_masks}

    all_sites = active_sites + inactive_sites
    chosen = random.sample(all_sites, min(n_visit, len(all_sites)))
    flips = {name: 0 for name in named_masks}

    for name, idx in chosen:
        h_val = int(named_masks[name][idx].item() > 0.5)
        s = float(parameter_saliency[name][idx].item())
        rho = get_param_rho(name, cfg)
        denom = rho / 2.0 + cfg.glauber_eps
        delta_tilde = (2 * h_val - 1) * (s / denom - 1.0)
        p_flip = _sigmoid_prob(delta_tilde, cfg.beta_h)
        if random.random() < p_flip:
            named_masks[name][idx] = 1.0 - named_masks[name][idx]
            flips[name] += 1
...
def glauber_update_neurons(
    ...
    """
    Finite-temperature Glauber updates for neuron gates g.

    Normalized energy difference:
      Δ̃E = (2g - 1) * ( S / (rho/2 + eps) - 1 )
    """
    active_g1 = [idx for idx in range(model.g1.numel()) if model.g1[idx] > 0.5]
    active_g2 = [idx for idx in range(model.g2.numel()) if model.g2[idx] > 0.5]
    inactive_g1 = [idx for idx in range(model.g1.numel()) if model.g1[idx] < 0.5]
    inactive_g2 = [idx for idx in range(model.g2.numel()) if model.g2[idx] < 0.5]

    all_g1 = [("g1", idx) for idx in active_g1 + inactive_g1]
    all_g2 = [("g2", idx) for idx in active_g2 + inactive_g2]

    if cfg.g1_site_fraction > 0.0 or cfg.g2_site_fraction > 0.0:
        n_g1 = int(math.floor(cfg.g1_site_fraction * len(all_g1)))
        n_g2 = int(math.floor(cfg.g2_site_fraction * len(all_g2)))
        chosen = []
        if n_g1 > 0 and len(all_g1) > 0:
            chosen.extend(random.sample(all_g1, min(n_g1, len(all_g1))))
        if n_g2 > 0 and len(all_g2) > 0:
            chosen.extend(random.sample(all_g2, min(n_g2, len(all_g2))))
    else:
        total_sites = len(all_g1) + len(all_g2)
        n_visit = int(math.floor(cfg.g_site_fraction * total_sites))
        if n_visit <= 0 or total_sites == 0:
            return {"g1": 0, "g2": 0}
        chosen = random.sample(all_g1 + all_g2, min(n_visit, total_sites))

    flips = {"g1": 0, "g2": 0}
    deaths = {"g1": 0, "g2": 0}
    rebirths = {"g1": 0, "g2": 0}
    for gname, idx in chosen:
        if gname == "g1":
            gate_tensor = model.g1
            name = f"g1[{idx}]"
        else:
            gate_tensor = model.g2
            name = f"g2[{idx}]"

        g_val = int(gate_tensor[idx].item() > 0.5)
        s = float(neuron_saliency[name])
        rho_g = get_neuron_rho(name, cfg)
        structural_cost = neuron_structural_cost(model, cfg, gname, idx)
        denom = rho_g / 2.0 + structural_cost + cfg.glauber_eps
        delta_tilde = (2 * g_val - 1) * (s / denom - 1.0)
        p_flip = _sigmoid_prob(delta_tilde, cfg.beta_g)
        if random.random() < p_flip:
            gate_tensor[idx] = 1.0 - gate_tensor[idx]
            flips[gname] += 1
            if g_val == 1:
                deaths[gname] += 1
            else:
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="LeNet-300-100 with consistent neuron-synapse Glauber dynamics")
